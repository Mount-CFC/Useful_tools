...
@author: lutaige   过渡态3.0 
由原来的选择电导区间变为选择距离区间,对单条曲线进行放大，配合hover2.0使用
"""
...
def hover(datacut, low_cond, high_cond, start_trace = 0, total_traces = 1):
    ...
    while True:
        ...
        for i in range(total_traces):
            ...
            total_single_trace.set_ylim(-6,1)
            ...
            '''生成单条悬停区间曲线'''
            trace = ax[i][2]
            trace.set_xlim(start_distance, end_distance)
            ...
            #选择满足x轴距离的y轴电导值
            filtered_y = y[(x >= start_distance) & (x <= end_distance)]
            ...
            ax[i][3].set_ylim(low_cond,high_cond)
...
if __name__ == '__main__':
    ...
    #%%
    '''统计参数设置'''
    start_distance = 0.75  #统计电导区间的开始距离
    end_distance = 1.4   #统计电导区间的结束距离
    
    '''显示参数设置'''
    low_cond = -4    #第2、3张图中显示电导区间的下限
    high_cond = -2     #第2、3张图中显示电导区间的上限
    Xlimit_single = 2  #第1张单条图完整曲线的x轴范围
    max_counts = 70   #第3张电导统计图的横坐标上限
